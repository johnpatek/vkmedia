...
static void
push_video_data(GstElement *appsrc, guint unused, void *data)
{
    ...
    if (G_LIKELY(sample))
    {
        ...
        GST_BUFFER_PTS(buffer) = gst_segment_to_running_time(seg, GST_FORMAT_TIME, pts);
        GST_BUFFER_DTS(buffer) = gst_segment_to_running_time(seg, GST_FORMAT_TIME, dts);
    ...
    gpointer user_data)
{
    ...
    ctx->generator_pipe =
        gst_parse_launch(
            "videotestsrc ! nvh264enc preset=5 zerolatency=true ! appsink name=vid max-buffers=2 drop=true ",
            NULL);
...
int main(int argc, char *argv[])
{
    ...
    gst_rtsp_media_factory_set_launch(factory,
                                      "( appsrc is-live=true name=videosrc ! queue ! rtph264pay name=pay0 pt=96 )");
